...
export const CreateBlView: React.FC<CreateBlViewProps> = ({
  ...
  const [numero, setNumero] = useState<string>(blToEdit?.numero || '');
        ...
        <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-xs space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="space-y-3">
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1.5">
                  N° BL <span className="text-slate-400 font-normal">(auto si vide)</span>
                </label>
                <input
                  type="text"
                  value={numero}
                  onChange={(e) => setNumero(e.target.value)}
                  placeholder="Ex. 2639/26"
                  className="w-full px-3 py-2 text-xs bg-white rounded-lg border border-slate-300 focus:outline-none focus:ring-2 focus:ring-emerald-500 font-mono"
                />
              </div>
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1.5">
