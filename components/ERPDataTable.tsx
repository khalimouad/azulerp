      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-xs overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs border-collapse">
            {/* Unified AeroTrack Enterprise Header */}
            <thead className={`${stickyHeader ? 'sticky top-0 z-10' : ''} bg-slate-50 text-slate-700 font-semibold text-xs divide-x divide-slate-200 border-b border-slate-200 select-none shadow-2xs`}>
              <tr>
                {/* Select All Checkbox */}
                {selectable && (
                  <th className="py-2.5 px-3 w-10 text-center bg-slate-50">
                    <button
                      type="button"
                      onClick={toggleSelectAllPage}
                      className="text-slate-400 hover:text-slate-700 transition cursor-pointer flex items-center justify-center mx-auto"
                ...
                {columns.map((col) => {
                  ...
                  return (
                    <th
                      ...
                      className={`py-2.5 px-3 text-xs font-semibold text-slate-700 ${alignClass} ${
                        isSortable
                          ? 'cursor-pointer hover:bg-slate-100 hover:text-slate-900 transition-colors'
                          : ''
                      } ${isSorted ? 'text-blue-700 bg-blue-50/50' : ''} ${col.className || ''}`}
                ...
                paginatedData.map((item, rowIdx) => {
                  ...
                  return (
                    <tr
                      ...
                      className={`divide-x divide-slate-200 border-b border-slate-200 transition-colors ${
                    ...
                    >
                      ...
                      {columns.map((col) => {
                        ...
                        return (
                          <td
                            key={col.key}
                            className={`py-2.5 px-3 text-xs text-slate-700 ${alignClass} ${col.className || ''}`}
                          >
                            {col.render
